 ...
 *
 * A grid with `repeat(auto-fit, minmax(96px, 1fr))` wraps correctly but leaves
 * the odd tile alone in a half-empty row, which on a phone is where two of
 * three of these land. Flex with `flex: 1 1 96px` wraps the same way and then
 * lets the stranded tile GROW to fill its row, so three tiles read as two small
 * and one wide rather than as a gap. No breakpoint, no media query, and no
 * arrangement in which a tile can overflow: the basis is a suggestion, not a
 * minimum, and `minWidth: 0` lets a long label break rather than push.
 *
