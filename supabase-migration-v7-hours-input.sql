...
-- WHY IT MATTERED
--
-- This file was committed in 836f227 and never run. Until it was applied,
-- total_hours was still whatever the browser sent: an employee could PATCH
-- their own pending row with {"total_hours": 999.99} and it stored verbatim,
-- because the RLS policies on work_hours constrain employee_id and status and
-- nothing else. Verified by probe against production before applying — it
-- stored 999.99 — and again after, where the same attempt held at 12.50,
-- recomputed from the row's own clock times.
--
-- Nobody had exploited it. All 252 rows' stored totals already matched the
-- formula exactly — 818.00 hours, zero drift — and applying this left them
-- unchanged.
--
...
-- skipping any, and the §6a normalisation passes were no-ops.
--
-- Adds:
